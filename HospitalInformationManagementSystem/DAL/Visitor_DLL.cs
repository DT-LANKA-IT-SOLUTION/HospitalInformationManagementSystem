    ...
    class Visitor_DLL
    {
        ...
        public static int UpdateVisitors(VisitorModel visitorModel)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(visitorModel.attachment_data)) 
                {
                    sql = string.Format("UPDATE Visitors SET " +
                    "purpose = @purpose," +
                    "fname = @fname," +
                    "lname = @lname," +
                    "phone_no = @phone_no," +
                ...
                else 
                {
                    ...
                    "note = @note," +
                    "attachment_type = @attachment_type," +
                    "attachment_data = @attachement_data," +
                    "user_id = @user_id " +
                    "WHERE visitor_id = @visitor_id"
                    ...
                    _sql[10] = SqlParameterFormat.Format("@attachment_type", visitorModel.cmbAttachmentType);
                    _sql[11] = SqlParameterFormat.Format("@attachement_data", file);
                }
        ...
        //        _sql[6] = SqlParameterFormat.Format("@in_time", visitorModel.in_time);
                _sql[6] = SqlParameterFormat.Format("@out_time", visitorModel.out_time);
                _sql[7] = SqlParameterFormat.Format("@note", visitorModel.note);
                _sql[8] = SqlParameterFormat.Format("@user_id", PermisionsModel.user_id);
                _sql[9] = SqlParameterFormat.Format("@visitor_id", visitorModel.visitor_id);
        ...
        public static SqlDataReader GetAttachment(int visitor_id)
        {
            string sql = string.Format("SELECT attachment_data FROM Visitors WHERE IsActive=IsActive AND visitor_id = @visitor_id");
